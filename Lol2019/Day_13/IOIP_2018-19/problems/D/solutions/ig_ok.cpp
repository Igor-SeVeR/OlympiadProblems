...
int main()
{
  ...
  while (!q.empty())
  {
    ...
    {
      if (it.it == 0)
        q.push({it.x1 + 1, it.x2, it.y1, it.y2, it.it});
      if (it.it == 1)
        q.push({it.x1, it.x2 - 1, it.y1, it.y2, it.it});
    }
    if (it.y1 + 1 <= it.y2)
    {
      if (it.it == 2)
        q.push({it.x1, it.x2, it.y1 + 1, it.y2, it.it});
      if (it.it == 3)
        q.push({it.x1, it.x2, it.y1, it.y2 - 1, it.it});
    }
    if (it.it + 1 < 4)
    {
      q.push({it.x1, it.x2, it.y1, it.y2, it.it + 1});
    }
  }
  cout << 0 << '\n';
  return 0;
